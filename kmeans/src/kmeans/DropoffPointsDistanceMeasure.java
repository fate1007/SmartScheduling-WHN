...
//                if (ConversionTest.monitored.contains(ll1) && ConversionTest.monitored.contains(ll2))
			...
			return Math.sqrt(Math.pow(ll1.getLat() - ll2.getLat(), 2) + Math.pow(ll1.getLon() - ll2.getLon(), 2));
			// return GeoPolygon.getEarthDistance(ll1.getLat(), ll1.getLon(),
			// ll2.getLat(), ll2.getLon());
        } else {
            double lineDist = GeoPolygon.getEarthDistance(ll1.getLat(), ll1.getLon(), ll2.getLat(), ll2.getLon());
            if (lineDist < 1)
                return lineDist * 1.1;
